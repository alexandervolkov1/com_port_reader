...
local function sine_value(generator, time)
    local angular_frequency = 2.0 * math.pi / generator.period
    return generator.amplitude * math.sin(angular_frequency * time + generator.phase)
end

local function noise_value(generator)
    if generator.noise_amplitude == 0.0 then return 0.0 end

    -- Uniform noise in [-noise_amplitude, +noise_amplitude].
    return generator.noise_amplitude * (2.0 * next_uniform(generator) - 1.0)
end
...
function read(instrument_id, parameter, time)
    ...
    if parameter == "value" then
        return sine_value(generator, time) + noise_value(generator)
...
function write(instrument_id, parameter, value, _time)
    local generator = get_generator(instrument_id)

    if parameter == "amplitude" then
        if value < 0.0 then error("sine amplitude must not be negative") end
        generator.amplitude = value
        return generator.amplitude
    elseif parameter == "noise_amplitude" then
        if value < 0.0 then error("noise amplitude must not be negative") end
        generator.noise_amplitude = value
        return generator.noise_amplitude
    elseif parameter == "period" then
        if value <= 0.0 then error("sine period must be greater than zero") end
